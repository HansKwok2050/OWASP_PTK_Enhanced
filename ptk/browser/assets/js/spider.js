...
import { ptk_controller_spider } from "../../../controller/spider.js"

const controller = new ptk_controller_spider()
let ascending = false
...
jQuery(function () {
    ...
    $(document).on('click', "#export_result", function (e) {
        controller.export_result().then(function (result) {
            if (result.record != null) {
                ...
                downloadLink.download = "PTK_spider_" + (new URL(result.record[0][0])).hostname + ".txt"
    ...
    $(document).on('click', "#clear_all", function (e) {
        controller.clear_all().then(function (result) {
            if (result.isScanning == false) { // if is scanning than does nothing
                let table = document.getElementById("spider_frames");
    ...
    document.getElementById('maxDepth').addEventListener('change', (event) => {
        controller.update_depth(event.target.value).then(function (result) {
        })
    });
...
async function bindInfo(result) {
    ...
    document.getElementById('maxDepth').value = result.depth;
...
browser.runtime.onMessage.addListener(function (message, sender, sendResponse) {
    if (message.channel == "ptk_background2popup_spider") {
        if (message.type == "scaner_updated") { 
        ...
        } else if (message.type == "scan_started") {
            document.getElementById('start_scan').style.display = 'none';
            document.getElementById('stop_scan').style.display = 'inline';
            document.getElementById('lock_url').style.display = 'none';
            document.getElementById('unlock_url').style.display = 'inline';
            document.getElementById('currentURL').classList.add("grayed-out");
            document.getElementById('currentURL').disabled = true;
            ...
            document.getElementById('currentURL').classList.remove("grayed-out");
            document.getElementById('currentURL').disabled = false;
        } 
    }
})
