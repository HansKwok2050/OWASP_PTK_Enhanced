...
jQuery(function () {
    ...
    $('#settings_save').on('click', function () {
        ...
        $form = $('#interceptor_form'), values = $form.form('get values')
        Object.keys(values).map((k) => { if (values[k] === 'on') values[k] = true })
        controller.save('interceptor', values)
...
$(document).on("init_forms", function (e, s) {
    ...
    Object.entries(s.interceptor).forEach(([key, value]) => {
        if ($('#interceptor_form').form('has field', key)) {
            $('#interceptor_form').form('set value', key, value)
        }
    })
